...
import com.jian.util.LayoutManagerType;
...
private int count;//所有的item总数；
    public LoadDataScrollController(int count) {
        this.count = count;
    }
    /**
     * 当前布局管理器的类型
    ...
    public LoadDataScrollController(OnRecycleRefreshListener onRecycleRefreshListener,int count) {
...
private int dy;
    ...
    public void onScrollStateChanged(RecyclerView recyclerView, int newState) {
        ...
        // 四个条件，分别是是否有数据，状态是否是滑动停止状态，是否已经显示全部数据,显示的最大条目是否大于整个数据（注意偏移量），是否正在加载数据
        if(visibleCount>0
                &&newState==RecyclerView.SCROLL_STATE_IDLE
                &&mLastVisibleItemPosition+1<count&&mLastVisibleItemPosition>=totalCount-1
                &&!isLoadData&&dy>0){
            //可以加载数据
            if(mListener!=null){
                isLoadData = true;
